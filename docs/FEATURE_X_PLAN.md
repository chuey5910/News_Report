...
5. ใช้ `province_filter.split_by_province()` ตรวจ title+summary (ฉบับไทย) ว่ามีชื่อจังหวัด (หรือคำพ้อง)
   ใน 17 จังหวัดหรือไม่ (ตั้งใจให้กว้าง/เน้นไม่พลาดข่าว — ข่าวที่แค่ "พาดพิงถึง" จังหวัดนั้น แม้ไม่ได้
   เกิดเหตุที่นั่นจริง ก็ยังนับว่า match) แบ่งผลเป็น 2 กลุ่ม:
   - **matched**: มีจังหวัดใน 17 จังหวัด match อย่างน้อย 1 จังหวัด (เก็บ tag จังหวัดไว้ ข่าวหนึ่งอาจ
     match หลายจังหวัด)
   - **general**: ไม่ match จังหวัดใดเลย — **ไม่ถูกทิ้ง** แต่เก็บไว้ในกลุ่มแยกต่างหาก (ดูหัวข้อ 7)
6. ทั้งสองกลุ่ม → `summarizer.py` ทำความสะอาด HTML, ตัดสรุปให้สั้นลง และแนบ **อ้างอิง**
   (ชื่อสำนักข่าวต้นฉบับ + ลิงก์ต้นฉบับ)
6.5. `trending.py` เทียบความคล้ายของหัวข้อข่าว (matched+general รวมกัน) ด้วย character n-gram
   Jaccard similarity (ไม่ใช้ AI) — ถ้าหัวข้อคล้ายกันมากพอและมาจาก**อย่างน้อย 3 สำนักข่าวที่ต่างกัน**
   ในรอบเดียวกัน จะติด tag `is_major_story` + `major_story_source_count` ให้ทุกข่าวในกลุ่มนั้น
   (แสดงเป็น badge "🔥 ข่าวใหญ่" ในเว็บแอป) — จำกัดแค่เทียบภายในรอบเดียวกัน (07:00 หรือ 16:00)
   ยังไม่เทียบข้ามรอบของวันเดียวกัน
7. บันทึกผลลัพธ์ของวันนั้นเป็น JSON (`data/reports/YYYY-MM-DD.json`) — `provinces` จัดกลุ่มตามจังหวัด,
   `general` เก็บข่าวที่ไม่ตรงจังหวัดใดเลย (แบบ flat list) — ถ้ามีรายงานของวันนั้นอยู่แล้ว (เช่น รอบ
   16:00 น. ในวันเดียวกับรอบ 07:00 น.) ทั้งสองกลุ่มจะ **merge** เข้าด้วยกัน ไม่เขียนทับ
...
10. บันทึก id ข่าวที่ประมวลผลแล้ว (ทั้ง matched และ general) ลง SQLite กันซ้ำในรอบถัดไป —
    ป้องกันไม่ให้ข่าว general ถูกดึง/แปลซ้ำทุกรอบทั้งที่ผลลัพธ์ (ไม่ match) จะเหมือนเดิม
